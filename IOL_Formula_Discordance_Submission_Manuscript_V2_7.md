...
**Main-text word count:** 2,878  
**Abstract word count:** 241  
**Tables/Figures:** 2/2
**Supplemental files:** 1 file containing supplemental methods and 13 supplemental tables
...
A local optical character recognition (OCR) pipeline identified formula pages and extracted eye, IOL model, formula name, IOL power, and predicted postoperative refraction. The archive comprised 38,226 pages from 5,194 examinations and yielded 908,037 traceable formula-power-prediction rows. Independent full-pipeline reruns produced identical normalized outputs. In a separate masked, OOD-enriched 500-eye source-screen audit, all prespecified biometry, formula, IOL model/power, and predicted-refraction fields matched the original reports. The audit sampling and field definitions are provided in the Supplement. Source-level sensitivities varied duplicate resolution and excluded low-confidence, conflicting, or sequence-inconsistent rows.
...
Source cleaning and anchor choice barely changed classification. Deterministic reruns and the separate source-screen audit supported extraction fidelity; rare errors outside the audited enriched sample remain possible.
