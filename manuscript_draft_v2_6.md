...
**Draft version:** 2.6, August 15, 2026  
**Main-text word count:** 2,875  
...
**Corresponding author:** Jinho Jeong, MD, Department of Ophthalmology, Jeju National University Hospital, 15 Aran 13-gil, Jeju-si, Jeju-do 63241, Republic of Korea. Email: AUTHOR COMPLETION REQUIRED  
...
**AI use declaration:** OpenAI Codex was used for data-checking support, statistical code review, language editing, figure preparation, and document formatting; the authors verified all outputs and take responsibility for the work.

> **PRIORITIZED AUTHOR ACTIONS BEFORE SUBMISSION - remove this box from the submitted manuscript.** Remaining gates: (1) archive the separate 500-eye source-screen audit protocol and deidentified field-level result sheet; (2) confirm IOLMaster 700 software-version history, formula constants, ARTIS model/fallback rule, TR1/TR2 definitions, and the 9 short-steep eyes' chart/topography and previous-refractive-surgery status; (3) provide study dates, IRB/waiver details, corresponding-author email, and the institutional authorization record for redistribution of the deidentified calibration geometry arrays; and (4) after authorization, tag and publish the metadata-only V3.2.1 package and DOI-backed archive. Before unmasking OOD500 outcomes, freeze its postoperative analysis plan and decide whether its prespecified validation will be nested in this report without using effect size to make that decision. Then remove this box and prepare the blinded submission package.
...
This retrospective observational study was conducted at Jeju National University Hospital and combined an institutional IOLMaster 700 biometry export with deidentified formula-screen images. Reporting followed the STROBE statement.8 AUTHOR COMPLETION REQUIRED - study dates and IRB approval/waiver details. Direct identifiers were retained only in a protected linkage environment. Analytic files used nonreversible patient and examination tokens.
...
Two investigators independently executed the complete OCR-to-CSV workflow on all 38,226 pages. After automatically generated execution timestamps were excluded, normalized sorted outputs were identical across all 908,037 rows. Separately from postoperative outcome entry, a resident performed source-screen verification in the masked 500-eye OOD review set. This set comprised all 34 Rare and 113 Uncommon eyes plus a deterministic SHA-256-seeded random sample of 353 Typical eyes from 2,500 recent eligible eyes; OOD category and percentile were concealed and review order was randomized. The resident directly compared the prespecified preloaded biometry fields and formula-screen fields (formula name, IOL model and power, and predicted refraction) with the original IOLMaster screens. Source-level sensitivities varied duplicate resolution and excluded low-confidence, conflicting, or sequence-inconsistent rows.
...
The separate source-screen audit found complete exact agreement across every manually verified biometry and formula-screen field in all 500 eyes (500/500; observed eye-level complete exact agreement, 100.0%; exact 95% CI, 99.3%-100.0%); no disagreement required adjudication. Across 16 nearby AL/K windows, all risk-ratio point estimates exceeded 5.2. In 3,356 eyes eligible under both anchors, high-versus-lower percentile risk ratios were 6.70 with Haigis and 7.31 with Barrett TK anchoring; continuous effects were 0.0218 and 0.0205 D per 10 percentiles. Expanded closest-time and minimum-confidence ≥0.95 analyses gave effects from 0.0212 to 0.0216 D per 10 percentiles.
...
Source cleaning and anchor choice barely changed classification. Identical full-pipeline OCR reruns established deterministic reproducibility, while the separate 500-eye manual audit established source-screen agreement across the audited biometry and formula-screen fields. Together, these complementary checks address workflow reproducibility and source fidelity without conflating the two.
...
The locally displayed panel mixed standard-K formulas with Barrett Universal II TK, so formula-specific inputs could contribute to spread. Positive leave-one-formula-out associations argue against a single-formula artifact but do not replace validation in a uniform modern panel. The leave-one-formula-out analysis was not designed to establish mechanism.

Formula spread does not identify the most accurate formula; postoperative refraction was unavailable. Because clinicians can already inspect formula-screen predictions, incremental decision benefit over unstructured review was not tested. A blinded 500-eye postoperative review using a locked score is underway, with its analysis plan to be frozen before unmasking. Both stages used one institutional source, so transportability across devices, software, constants, IOLs, and populations remains unknown. The 500-eye manual audit found no discrepancies in the audited biometry or formula-screen fields; because the sample was intentionally OOD-enriched, rare errors outside the audited sample cannot be excluded. Selection removed longer and more atypical eyes, and generalization beyond conventional AL is unsupported.

Measurement provenance remains important. Alternate record rules changed 0.31% to 0.44% of flags, but 6 of 40 events at ≥0.75 D arose among 9 unreviewed short-steep eyes. Their charts/topography and previous-refractive-surgery status require review. Nonsignificant year estimates do not replace software and constant documentation; calibration-array release still requires governance.
...
- A biometry-only model frozen before formula outcomes tested whether individually plausible measurements fit together; its association was graded across the score distribution and persisted in patients assigned to the model's untouched test split.
...
Analysis code and the V3.2 source and model artifact are available at https://github.com/peopleatkorea-droid/IOLmaster_conversion. The tagged source revision `biometry-ood-v3.2.0` resolves to commit `97e3d50`. As accessed August 15, 2026, the stable calculator at https://k-era.org/tools/biometry-ood served `continuous-age-bilateral-v3.2.0`; its 7-file public bundle has an immutable SHA-256 manifest and synthetic educational demos only. Apache-2.0 licensing, citation metadata, Zenodo metadata, and an institutional release record have been prepared for a metadata-only V3.2.1 packaging release. The V3.2.1 tag, stable promotion, and DOI-backed deposit will follow documented institutional authorization for redistribution of the deidentified calibration geometry arrays. The V3.2 calculator also reports a separate age-and-AL-conditional geometry score that was not analyzed in this study.
...
Supplemental Tables S1-S3 describe outcomes, cohort assembly, and the frozen design; S5-S8 and S10-S12 report robustness or internal validation. S4, S9, S11, and S13 are explicitly exploratory or post hoc. Continuous spread was the primary outcome, spread ≥0.75 D was prespecified secondary, and the conventional-AL/K 0.50-D contrast was post hoc.
